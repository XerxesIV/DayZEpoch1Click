// included compiles
call compile preprocessFileLineNumbers "EpochMods\ClickActions\vehicles\config.sqf";
call compile preprocessFileLineNumbers "EpochMods\ClickActions\vehicles\wrapper.sqf";
call compile preprocessFileLineNumbers "EpochMods\ClickActions\vehicles\functions.sqf";
...
player_deploy = compile preprocessFileLineNumbers "EpochMods\ClickActions\vehicles\player_deploy.sqf";
...
[] spawn {
    ...
    {DZE_CLICK_ACTIONS = DZE_CLICK_ACTIONS + [[(_forEachIndex call getDeployableKitClass),format["Deploy %1",(_forEachIndex call getDeployableDisplay)],format["%1 execVM 'EpochMods\ClickActions\vehicles\deploy.sqf';",_forEachIndex],"true"]];} forEach DZE_DEPLOYABLES;
    DZE_DEPLOYING      = false;
    DZE_PACKING        = false;
            ...
            {   
                ...
                if(!(isNull _cursorTarget)
                    ...
                    if ((_forEachIndex call getActionId) < 0) then {
                        [_forEachIndex,player addaction["<t color='#33b5e5'>" + format["Pack %1",(_forEachIndex call getDeployableDisplay)] + "</t>","EpochMods\ClickActions\vehicles\pack.sqf",[_forEachIndex,_cursorTarget],0,false,true,"", ""]] call setActionId;
                    };
                } else {
